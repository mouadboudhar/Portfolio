...
const TypeBadge = ({ type }: { type: CyberProject['type'] }) => {
  ...
  return (
    <span className={`px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs font-mono rounded-full border ${config[type].color}`}>
      {config[type].label}
    </span>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
          ...
          <div className="flex gap-2">
            ...
            {project.link && (
              <motion.a
                ...
                whileTap={{ scale: 0.9 }}
              >
                {project.type === 'report' ? <FiFileText className="w-4 h-4 sm:w-[18px] sm:h-[18px]" /> : <FiExternalLink className="w-4 h-4 sm:w-[18px] sm:h-[18px]" />}
              </motion.a>
            )}
          </div>
        </div>
        ...
        {/* Title */}
        <h3 className="font-mono text-base sm:text-lg font-semibold text-white mb-1 sm:mb-2 group-hover:text-[#00ff9d] transition-colors">
          {project.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2">
          {project.tags.map((tag) => (
            <span 
              key={tag}
              className="px-1.5 sm:px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs font-mono text-gray-500 bg-[#0a0a0a] rounded"
            >
              {tag}
            </span>
          ))}
...
const CyberProjects = () => {
  return (
    <Section id="cyber-projects" variant="cyber">
      {/* Section Header */}
      <motion.div 
        className="w-full max-w-5xl mb-6 sm:mb-12 text-center"
        variants={fadeInUp}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: false, amount: 0.3 }}
      >
        <div className="inline-flex items-center gap-2 mb-2 sm:mb-4 font-mono text-xs sm:text-sm text-gray-500">
          <span className="text-[#00ff9d]">❯</span>
          <span>ls ./security-projects/</span>
        </div>
        
        <h2 className="font-mono text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-2 sm:mb-4">
          Security Projects
          <span className="animate-pulse text-[#00ff9d]">_</span>
        </h2>
        
        <p className="font-mono text-xs sm:text-sm text-gray-500 max-w-2xl mx-auto">
